function [hs_rmse_ww3_br, hs_rmse_pred_br, tm_rmse_ww3, tm_rmse_pred, ...
...
for fh = 1:num_hours
    bias = nanmean(pred(fh).hs - obs.hs);
    hs_rmse_pred(fh) = sqrt(nanmean((pred(fh).hs - obs.hs).^2));
    hs_rmse_pred_br(fh) = sqrt(nanmean((pred(fh).hs - obs.hs-bias).^2));
end
