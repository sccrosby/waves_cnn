# clip forecast -h to forecast_len to match the format of hindcast
# to allow buoys 46015, 46027, 46029, 46089 with forecast data to be pretrained
# and fine tuned on buoys 46211, 46214, 46218 with hindcast data 
#
# inputs:
    ...
    def calc_valid_start_indices(self):
        ...
        for i in range(len(self.time_zulu)-1):
            ...
            if (i*6-h >= 0 and i*6-h < self.obs_tensor.shape[2]
                and self.observed_time[i*6-h] - self.time_zulu[i-(h//6)] < 0.0417):
                test5 = True
            # valid obs end index
        ...
        print("valid start indices:", len(valid_indices))
...
#            model.load_state_dict(torch.load(model_path, map_location=device))
        ...
        valid_start = self.calc_valid_start_indices(self)
        ...
        # (hour x N x 5 x q)
        hour_buckets = np.empty((f, len(valid_start)+h_f, 5, q))
        hour_buckets[:] = np.nan
        ...
        test_num_batches = 0
        for i,idx in enumerate(valid_start):
            # get our inputs (MB x (2H + F) x 5 x Q)
            temp = self.obs_tensor[:, :, idx*6-h:idx*6]
            for j in range(h):
                inputs[0, j, :, :] = temp[:, :, j]
            temp = self.for_tensor[:, :, 0:h, idx-(h//6)]
            for j in range(h):
                inputs[0, j, :, :] = temp[:, :, j]

            temp = self.for_tensor[:, :, 0:f, idx]
            for j in range(f):
                inputs[0, j, :, :] = temp[:, :, j]
            ...
            with torch.no_grad():
                if on_gpu:
                    ...
                    predictions, _ = model(inputs_t, y_for_t, mean, std)
        ...
        def create_data_dict(d):
            data_dict = {
                ...
                'b2': d[:, 3, :],
                'e':  d[:, 4, :],
                'obstime': self.observed_time,
                'timezulu': self.time_zulu,
        ...
        data_dict = create_data_dict(self.reverse_pre_processing_obs(self.obs_tensor))
        curr_file = target_dir + time_stamp + '_obs_data.mat'
        self.generate_mat_lab_file(curr_file, data_dict)

        # Generate the for file
        data_dict = create_data_dict(self.reverse_pre_processing_for(self.for_tensor))
    ...
    def reverse_pre_processing_obs(self, tensor_object):
        # reverse standardization (* self.std + self.mean)
        tensor_object[E, :, :] = (tensor_object[E, :, :].transpose() * self.std[E, :] + self.mean[E, :]).transpose()
    ...
    def reverse_pre_processing_for(self, tensor_object):
        # reverse standardization (* self.std + self.mean)
        for i in range(self.for_tensor.shape[-1]):
            tensor_object[E, :, :, i] = (tensor_object[E, :, :, i].transpose() * self.std[E, :] + self.mean[E, :]).transpose()
