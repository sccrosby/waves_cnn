function [ obs, ww3, pred] = load_data( bouy, bnum, fol, prefix, num_hours )
%UNTITLED2 Summary of this function goes here
%   Detailed explanation goes here

obs = load(sprintf('../noah_research/%s/%s/%s/%s_obs_data.mat', bouy, bnum, fol, prefix));
ww3 = load(sprintf('../noah_research/%s/%s/%s/%s_ww3_data.mat', bouy, bnum, fol, prefix));
for hr = 1:num_hours
    pred(hr) = load(sprintf('../noah_research/%s/%s/%s/%s_hr%d_data.mat',bouy, bnum, fol,prefix,hr));
end
