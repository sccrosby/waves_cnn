...
class ConvNN(nn.Module):
    ...
    def apply_moment_constraints(self, mean_e, std_e, ww3_e, model_e, std_x, mean_x, ww3_x, model_x):
        # 0425 constraint model
        # cutoff any negative energy
        e_min = -1.0 * mean_e / std_e
        e_hat = torch.max(ww3_e + model_e, e_min).unsqueeze(2)
    ...
    def forward(self, inputs, y_ww3, mean, std):
        ...
        # add residuals to ww3
        pred = self.conv_out(pred) + y_ww3
