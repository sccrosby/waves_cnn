...
# FUTURE ROLLO: THESE PROBABLY SHOULD NOT ALL BE GLOBALS BUT IT'S OK FOR NOW
buoy = BUOY
history = tm['h']
forecast = tm['f']
q_freq = [0.04, 0.25]
...
class Trainer:
    ...
    # @ e.xcapture
    # ROLLO !!!! REMOVED _run AS SECOND PARAM FOR THIS FUNCTION WHEN GUTTING SACRED
    def train(self, best_model_fn, q_freq, md_lambda, energy_lambda, logger):
    ...
    def run(self):
        best_loss = self.train(best_model_fn, q_freq, md_lambda, energy_lambda, logger)
        return best_loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--name', type=str, default='test-exp')
    parser.add_argument('--project', type=str, default='test-exp')
    parser.add_argument('--entity', type=str, default=None)

    args = parser.parse_args()

    logger = WandBLogger(args.project, args.name, entity=args.entity)
    ...
    for i in range(num_experiments):
        ...
        # loss = start_train(params).result
        loss = start_train(params, logger)
